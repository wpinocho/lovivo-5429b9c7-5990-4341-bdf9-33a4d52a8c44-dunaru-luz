...
- **Cera Duna** = nombre propietario de la cera (vegetal, en gránulos, sin parafina). ✅ **2026-08-25: ya migrado también a los TÍTULOS Y DESCRIPCIONES DE LA DB.** Ya no queda "Cera Perlada" en el catálogo.
- **Cera 100% vegetal, biodegradable y ecológica** (claim aprobado por la owner). ⛔ NO usar porcentajes de soya/coco (sin confirmar).
...
### 🏷️ SISTEMA DE NOMBRES (2026-08-25 — nomenclatura premium vigente)
Regla: **`[Qué es] · [Formato]`**. Nada de "Kit", "Pack" ni "Recarga" (suenan a consumible barato).
El gramaje y las mechas viven en el acordeón "Qué incluye", no en el título.
...
⚠️ El título vive en la DB. `CATALOG_FALLBACK` de `IndexUI.tsx` y el footer de `EcommerceTemplate.tsx` tienen copias hardcodeadas: **si se renombra en el Dashboard hay que sincronizarlas.**
...
- **MECHAS (confirmado por la owner 2026-08-25): 30 por bolsa de 500 g → Dúo 60, Trío 90, Cera Duna 1 kg = 60.**
- **COLECCIONES**: `kits` (2), `recipientes` (2), `accesorios` (1). Falta **`recargas`**.
- **ORDEN DE MERCHANDISING** = `src/lib/catalog-order.ts` (grupos: Empieza aquí · Cera Duna · Colecciones de tonos · Recipientes y accesorios).
...
- **📚 ACORDEONES DE LA PDP (3, orden fijo)**: `Qué incluye` (driven por `src/lib/pdp-includes.ts`) → `La pieza` (description de la DB) → `Envío y garantía`. Todos cerrados por default. ⛔ **"Cuidado y seguridad" fue eliminado el 2026-08-25 por decisión de la owner: no aporta valor. No volver a añadirlo.**
...
- **🏠 ORDEN DE LA HOME** (`IndexUI.tsx`): hero → barra de credenciales → 4 pasos → "Elige tu vela" → `RitualSection` → `Reviews` → `ScentsSection` → "Elige tu tono" → `CasaRealSection` → `BrandStorySection` → **`<CompareTable />` "dunaru vs vela normal"** → FAQ → newsletter.
  - 🔁 2026-08-25: la tabla comparativa se BAJÓ (antes iba tras los 4 pasos). Va después de la historia de marca porque es un bloque de resolución de objeciones, no de descubrimiento. Fondo `bg-dunaru-arena texture-arena` para separarla del FAQ (`bg-background`).
...
- Constantes reutilizadas: `CERA_500`, `MECHAS_30`, `ENVIO`. Cubre los 10 slugs.
- ⛔ Solo datos verificados: **no** afirmar "guía impresa", "sin plomo" ni composición exacta.
...
**Estado**: ✅ 5.0–5.3 · ✅ **5.3.5 nomenclatura premium del catálogo** · 🔜 **SIGUIENTE: 5.4 (tiers con nombre y % de ahorro) y verificación en móvil 360 px.**
...
3. ✅ **Cera 100% vegetal, biodegradable y ecológica.** ⛔ Sin porcentajes de soya/coco.
4. ✅ **Mechas: 30 por bolsa. Dúo 60, Trío 90.**
5. ✅ **Nomenclatura premium aprobada** (ver tabla en la sección 1).
6. ❓ Nombre de la garantía (`Garantía Primera Luz · 30 días`).
7. ❓ ¿Cuántas horas da UNA mecha? (Solira dice 15 h).
8. ❓ Copy del empaque nuevo (bolsa resellable, caja, inserto). ❓ ¿Hay inserto impreso?
9. ❓ ¿B2B / wholesale? ❓ ¿SKU sample barato (30 g + 5 mechas)?
...
## 4. Recent Changes
- 2026-08-25 — 🏷️ **NOMENCLATURA PREMIUM: renombrados 8 productos en la DB** (título + descripción). "Recarga…" → **Cera Duna · 500 g / 1 kg**, "Kit Vela Rellenable" → **Vela Rellenable · Vaso de Vidrio / Bowl de Cerámica**, "Pack de 30 Mechas" → **30 Mechas de Algodón**, "Bowl de Cerámica Negro" → **Bowl Artesanal de Cerámica**. Slugs intactos. `compare_at_price` se conservó en todos.
- 2026-08-25 — 🔻 **Tabla comparativa bajada en la home**: ahora va después de `BrandStorySection`, antes del FAQ, con fondo arena.
- 2026-08-25 — ✂️ **Acordeón "Cuidado y seguridad" eliminado de la PDP** (decisión de la owner). Quedan 3 acordeones.
- 2026-08-25 — 🔢 **Mechas corregidas: Trío = 90** (FAQ de la home, FAQ de la PDP del Trío) y **Cera Duna 1 kg = 60** (antes decía 30 en la PDP).
- 2026-08-25 — 🧹 Footer, `CATALOG_FALLBACK` de la home y FAQs de la PDP sincronizados con los nombres nuevos; "cera perlada" eliminado del copy.
- 2026-08-25 — ⚖️ **`CompareTable.tsx`: tabla comparativa extraída a componente único** y montada también en la home.
- 2026-08-25 — 📦 **Acordeón "Qué incluye"** (`src/lib/pdp-includes.ts`, los 10 slugs).
- 2026-08-25 — 📚 Acordeones renombrados: `Descripción` → **La pieza**, `Envío y devoluciones` → **Envío y garantía**.
- 2026-08-25 — 🌱 **Claim "Cera 100% vegetal y biodegradable"** en credenciales, FAQ, `BrandStorySection` y tablas comparativas.
- 2026-08-25 — 🌾 **"Cera Duna": la cera ya tiene nombre propietario.**
- 2026-08-25 — 🔴→✅ Eliminada la contradicción del aroma. Bloque "Seis aromas, una sola vela".
...
- 🟡 Imágenes subidas por el owner sin usar aún: `1787681082141-dy7wr0dcp15.webp`, `1787681082142-42qlfq25nvs.webp`, `1787684660654-vr5uiznl7cj.webp` (revisar y asignar slot).
...
## 6. Known Issues
- 2026-08-25 — 🟠 **Los nombres nuevos NO están en los anuncios de Meta ni en emails automatizados.** Si un anuncio dice "Recarga 500 g" hay contradicción con la landing. Avisar al owner.
- 2026-08-25 — 🟡 `CATALOG_FALLBACK` (IndexUI) y el footer de `EcommerceTemplate` duplican los títulos: si se renombra en el Dashboard hay que sincronizar el código.
- 2026-08-25 — 🟡 **Sin verificar en 360 px**: `CompareTable` en la home (3 columnas) y `ScentsSection`.
- 2026-08-25 — 🟡 La descripción de `bowl-negro` y `vaso-extra-transparente` todavía dice "perlas dunaru" en vez de "Cera Duna".
...
- 2026-08-20 — 🟠 `ProductStorySections.tsx` aún tiene `dunaru-champagne` en los bullets. 🟠 Trust-icons de MSI en champagne/ambar. 🟠 Paleta sin auditar en carrito ni checkout.
- 2026-08-07 — 🟠 Escalera de precio por gramo rota: Dúo 1 kg ($1.10/g) vs Cera Duna 1 kg ($0.80/g).
- 2026-08-07 — 🟡 `getReviewStats()` es global (4.9/15), no por SKU. 🟠 Slug `kit-vaso-de-concreto` incorrecto (dice concreto, es cerámica). 🟡 `ShippingPromise` asume gratis si `shippingCost === 0`. 🟡 `PDP_BENEFITS` solo cubre 6 slugs.
- 2026-07-31 — 🔴 `ecommerce--update-product` NO persiste `compare_at_price` cuando se envía explícitamente. Workaround: Dashboard manual. (Sí se CONSERVA si no se envía el campo.)
...
- [ALTA] **Verificar en móvil (360 px)**: `CompareTable` en la home, `ScentsSection`, los 3 acordeones de la PDP.
...
- [MED] Limpiar "perlas dunaru" de las descripciones de `bowl-negro` y `vaso-extra-transparente`.
