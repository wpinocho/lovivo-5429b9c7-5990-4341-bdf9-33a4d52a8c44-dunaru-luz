...
function FeaturedCard({ r }: { r: Review }) {
  return (
    <figure className="h-full bg-background rounded-lg overflow-hidden border border-border/50 flex flex-col">
      ...
      <figcaption className="p-4 sm:p-5 flex flex-col gap-2 flex-1">
  ...
  return (
    <section id="resenas" className="py-12 sm:py-16 bg-dunaru-arena scroll-mt-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Resumen: promedio a la izquierda · distribución a la derecha */}
        <div className="max-w-lg mx-auto mb-6 flex flex-row items-center gap-5 sm:gap-8">
          <div className="flex flex-col items-center shrink-0">
            <span className="font-display text-4xl sm:text-5xl text-foreground leading-none">{average}</span>
            <Stars rating={Math.round(average)} className="mt-1.5" />
            <span className="font-body text-xs text-foreground/60 mt-1">{count} opiniones</span>
          </div>
